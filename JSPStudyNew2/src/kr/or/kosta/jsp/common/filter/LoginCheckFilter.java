...
//	private String loginPage = "/user/login.jsp";
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if(isLogin) {
			...
			request.setAttribute("uri", ((HttpServletRequest)request).getRequestURI()); //동적으로 uri 담아주는 것
			request.getServletContext().getRequestDispatcher(loginPage).forward(request, response); //동적으로 uri 담아주는 ㅇ
		}
	}
